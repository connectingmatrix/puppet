...
const readEvent = (page, name, event) => name === 'console' || name === 'pageerror' ? new ConsoleMessage(page, event) : name === 'request' ? new RequestHandle(page, event) : event;
const matchConsole = (match, event) => !match ? true : match && match.call && match.apply ? Boolean(match(event)) : match && match.test && match.exec ? match.test(event.text()) : event.text().includes(`${match}`);
...
const runDebugger = (page, action = 'start', script = '') => requestJson(page.baseUrl, '/api/pages/debugger', 'POST', { action, pageId: page.pageId, raw: true, script });
const readEvents = async (page, name, options = {}) => (await requestJson(page.baseUrl, '/api/pages/events', 'POST', { limit: options.limit || 200, name, pageId: page.pageId, raw: true })).items || [];

export class Page {
    constructor(browser, item, frameId = 0) {
        this.baseUrl = browser.baseUrl; this.browser = browser; this.console = { on: (handler) => this.on('console', handler), read: (options = {}) => this.consoleMessages(options), write: (script = '') => runDebugger(this, 'evaluate', script) }; this.debugger = { start: () => runDebugger(this, 'start'), stop: () => runDebugger(this, 'stop') }; this.frameId = frameId; this.iframe = new Proxy((value) => this.frame(value), { get: (_target, key) => this.frame(Number(key) || 0) }); this.keyboard = new Keyboard(this); this.localStorage = new LocalStorageStore(this); this.network = { requests: (options = {}) => readEvents(this, 'network.request', options), socket: (options = {}) => readEvents(this, 'socket', options) }; this.request = readPageRequestClient(this); this.socket = { on: (handler) => this.on('socket', handler) };
    ...
    async consoleMessages(options = {}) { const data = await requestJson(this.baseUrl, '/api/pages/console', 'POST', { limit: options.limit || 80, pageId: this.pageId, raw: true }); const items = []; for (const item of data.items || []) items.push(new ConsoleMessage(this, item)); return items; }
    async waitForConsole(match = '', options = {}) { for (const item of await this.consoleMessages(options)) if (matchConsole(match, item)) return item; return new Promise((resolve, reject) => { const stop = this.on(options.eventName || 'console', (event) => { if (!matchConsole(match, event)) return; clearTimeout(timer); stop(); resolve(event); }); const timer = setTimeout(() => { stop(); reject(new Error(`Timed out waiting for ${options.eventName || 'console'}.`)); }, Number(options.timeoutMs || 45000)); }); }
