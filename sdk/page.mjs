...
export class Page {
    constructor(browser, item, frameId = 0) {
        ...
        this.pageId = item.pageId; this.pageName = item.pageName || item.title || ''; this.pageStats = item.pageStats || { cpu: 0, heapUsage: 0, ram: 0 }; this.pageUrl = item.pageUrl || item.url || ''; this.sessionId = item.sessionId || browser.sessionId; this.tabId = item.tabId || 0;
    }
    ...
    async screenshot(options = {}) { const data = await requestJson(this.baseUrl, '/api/pages/screenshot', 'POST', { fullPage: options.fullPage || false, pageId: this.pageId, selector: options.selector || '' }); await saveBase64(options.path || '', data.dataBase64 || ''); return data; }
