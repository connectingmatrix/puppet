...
export const runRemoteJob = async (job: RemoteJob, instanceId: string, emit: LiveEmit) => {
    ...
    if (job.kind === 'pages-screenshot') return readLiveShot((job.payload as any).pageId, (job.payload as any).selector || '', Boolean((job.payload as any).current) || (job.payload as any).fullPage === false, job.payload as any);
