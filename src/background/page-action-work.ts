...
export const runLiveAction = async (action: PageAction, emit: LiveEmit) => {
    ...
    if (action.type === 'screenshot_page') return readResult(action, await readLiveShot(action.pageId || '', action.selector || '', Boolean(action.current) || action.fullPage === false, action));
